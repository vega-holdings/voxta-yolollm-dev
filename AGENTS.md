...
- Graph extraction: during summarization, YOLOLLM runs a separate LLM call (prompt `GraphExtractionPromptPath`) and writes a `GRAPH_JSON:` inbox file to `Data/GraphMemory/Inbox` (injects `meta.chatId/sessionId/user/characters` so GraphMemory can scope graph writes). This avoids storing graph payloads in character memory books.
